...
Run a production server on loopback, then run `pnpm test:map:browser`. The runner defaults to `http://127.0.0.1:4317`; set `RUNWAY_BASE_URL` for another production origin and `RUNWAY_EVIDENCE_DIR` to retain a prior run. It writes `baseline.json`, one deterministic JSON log per case, screenshots, and the bounded 30-second browser animation-frame trace.

The runner uses isolated contexts for each cold navigation and reload. It records readiness, page crashes, page errors, console errors, failed requests and HTTP failures. It still writes evidence after errors and returns nonzero after all cases close.
