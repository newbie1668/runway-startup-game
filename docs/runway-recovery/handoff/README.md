...
- Accepted source is on the branch head. No worker branches are waiting.
  ...
  as `ead3fb5`, with review fix `77fdd79`.
- The faithful-London track (F1–F6, G2–G4) is not part of PR #30 and needs
  input from a person: the photo pass, Foo's review and a physical iPhone.
- The previous handoff and running log are archived in
  [history/](../history/). Other files in this folder describe the completed
  cover-index task and are historical.
