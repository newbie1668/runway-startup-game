...
const completed = createCellStockJob({ id: 'complete', generation: 1, essential: true, cityData: city, cell, excludedBuildingIndices: new Set(), material, detail: 'overview', now: () => 0, onReady: (v) => { complete = v.group; } });
drain(completed); const geometry = (complete!.children[0] as THREE.Mesh).geometry; let disposed = 0; geometry.addEventListener('dispose', () => disposed++); completed.cancel(); assert.equal(disposed, 0);
